...
import {
  ...
  debounceSavePageFast,
  flushPendingSave,
} from '@/db';
import { createEmptyLine, getPageLineLimit, applyPageModeTransition } from '@/lib/paginationTransition';
...
export interface DraftingEngineSlice {
  // Cursor & platen layout
...
export const createDraftingEngineSlice: StateCreator<
  TypingStore,
  [],
  [],
  DraftingEngineSlice
> = (set, get) => ({
  ...
  setActiveColumnLimit: (limit: number) => {
    ...
    if (state.manifest?.id) {
      debounceSavePageFast(state.manifest.id, state.currentPageNumber, currentReflow.lines);
  ...
  insertChar: (char: string) => {
    ...
    if (!needsWrap) {
      ...
      if (currentManifest.mode === 'local') {
        // Fast debounced page save without deep PageRecord clone
        debounceSavePageFast(currentManifest.id, state.currentPageNumber, lines);
      }
    ...
    if (state.manifest.mode === 'local') {
      debounceSavePageFast(state.manifest.id, state.currentPageNumber, lines);
    }
      ...
      if (printableIndex >= 0) {
        ...
        if (prevLine && prevLine.wrapType === 'hard' && !prevLine.isSessionDivider) {
          ...
          set({
            ...
            activeLineWords: uncommittedLineWords,
          });
          return;
        }
