...
export const PrintModal: React.FC<PrintModalProps> = ({
  ...
  const [isConfirmingNewSession, setIsConfirmingNewSession] = useState<boolean>(false);
      ...
      >
        ...
        <div className="w-8 sm:w-10 shrink-0 flex flex-col border-r border-border/60 bg-muted/25 h-full select-none divide-y divide-border/40">
          ...
          <button
            ...
            title="Files: Saved Projects Explorer"
          >
          ...
          <div className={`flex-1 min-h-0 flex-col gap-2.5 sm:gap-3 overflow-hidden ${activeTab === 'document' ? 'flex' : 'hidden'}`}>
            ...
            <div className="relative flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-border/60 text-xs font-sans shrink-0">
              ...
              <div className="relative">
                <button
                  type="button"
                  onClick={() => setIsConfirmingNewSession(!isConfirmingNewSession)}
                ...
                {isConfirmingNewSession && (
                  <div className="absolute bottom-11 left-0 z-30 p-4 rounded-none border border-border bg-popover text-popover-foreground shadow-2xl flex flex-col gap-3 w-72 font-sans text-xs animate-in fade-in zoom-in-95 duration-100">
                    <p className="font-bold text-popover-foreground">Start a new session?</p>
                    <p className="text-muted-foreground text-[11px] leading-relaxed">
                      Current drafting will be sealed as a completed session, and a new session will begin at the end of the manuscript.
                    </p>
                    <div className="flex items-center justify-end gap-2 pt-1 border-t border-border/40">
                      <button
                        type="button"
                        onClick={() => setIsConfirmingNewSession(false)}
