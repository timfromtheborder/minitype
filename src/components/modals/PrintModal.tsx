...
export const PrintModal: React.FC<PrintModalProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Tall and Skinny Tabs with Sideways Vertical Text on Left Side */}
        <div className="w-8 sm:w-10 shrink-0 flex flex-col border-r border-border/60 bg-muted/25 h-full select-none">
          {/* Top Tab: Document (fills 50% height) */}
          <button
            type="button"
            onClick={() => setActiveTab('document')}
            className={`flex-1 flex flex-col items-center justify-center py-4 cursor-pointer transition-colors border-b border-border/40 ${
              activeTab === 'document'
                ? 'bg-background text-foreground font-bold border-r-2 border-r-primary'
                : 'text-muted-foreground hover:text-foreground hover:bg-muted/40'
            }`}
            title="Document Preview & Export"
          >
            <FileText className="w-3.5 h-3.5 mb-3 shrink-0" />
          ...
          {/* Bottom Tab: Files (fills 50% height) */}
          <button
            type="button"
            onClick={() => setActiveTab('files')}
            className={`flex-1 flex flex-col items-center justify-center py-4 cursor-pointer transition-colors ${
              activeTab === 'files'
                ? 'bg-background text-foreground font-bold border-r-2 border-r-primary'
                : 'text-muted-foreground hover:text-foreground hover:bg-muted/40'
            }`}
          ...
          >
            <FolderOpen className="w-3.5 h-3.5 mb-3 shrink-0" />
            <span className="[writing-mode:vertical-rl] rotate-180 text-[10px] sm:text-[11px] font-sans tracking-widest uppercase font-semibold">
              Files
            </span>
          </button>
        ...
        <div className="flex-1 min-w-0 flex flex-col p-3 sm:p-5 gap-2.5 sm:gap-3 overflow-hidden">
          ...
          <div className={`flex-1 min-h-0 flex-col gap-2.5 sm:gap-3 overflow-hidden ${activeTab === 'document' ? 'flex' : 'hidden'}`}>
            ...
            <div className="relative flex items-center justify-between pt-2 border-t border-border/60 text-xs font-sans shrink-0">
              {/* New Document Button with Pop-Over Confirmation */}
